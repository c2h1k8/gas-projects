...
const notificationMenuDisp = (replyToken) => {
  ...
  const now = new Date();
  const minDate = DateUtils.formatDate(new Date(now.getFullYear(), now.getMonth(), 1), 'yyyy-MM-dd');
  const maxDate = DateUtils.formatDate(new Date(now.getFullYear(), now.getMonth() + 1, 0), 'yyyy-MM-dd');
  columns.push(LineUtil.getCarouselColumn('https://placehold.jp/001df5/ffffff/640x480.png?text=カレンダー', '', '入力する勤怠を選択してください。', [
    {
      'type': 'datetimepicker',
      'label': '出退勤',
      'data': JSON.stringify({ 'action': 'calendar', 'type': '出勤' }),
      'mode': 'datetime',
      'min': minDate + 'T00:00',
      'max': maxDate + 'T23:59',
    },
    {
      'type': 'datetimepicker',
      'label': '欠勤',
      'data': JSON.stringify({ 'action': 'calendar', 'type': '欠勤' }),
      'mode': 'date',
      'min': minDate,
      'max': maxDate,
    },
    {
      'type': 'datetimepicker',
      'label': 'クリア',
      'data': JSON.stringify({ 'action': 'calendar', 'type': 'クリア' }),
      'mode': 'date',
      'min': minDate,
      'max': maxDate,
    },
  ...
  LineManager.replyCarousel(replyToken, 'メニューが表示されました。', columns);
}
